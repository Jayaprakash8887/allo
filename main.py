...
llm_base_path = get_config_value("llm", "llm_base_path", None)
...
# llm = Ollama(base_url='http://152.67.162.156:11434', model="mistral")
...
def invoke_llm(user_id, language, current_session_id, user_input) -> GetContentResponse:
    ...
    chat_history_messages = retrieve_data(current_session_id + "_welcome")
    ...
    logger.info({"user_id": user_id, "user_language": language, "messages": messages})
    ...
    chat_history_messages.append({"role": "assistant", "content": ai_assistant})
    store_data(current_session_id + "_welcome", json.dumps(chat_history_messages))
    ...
    if "user_agreed" in ai_assistant.lower() or "user\\_agreed" in ai_assistant.lower():
    ...
    logger.info({"user_id": user_id, "user_language": language, "content_response": content_response})
    return content_response


# def invoke_llm_feedback_old(user_id, language, current_session_id, user_input) -> GetContentResponse:
#     # setup Redis as a message store
...
def invoke_llm_feedback(user_id, language, current_session_id, user_input) -> GetContentResponse:
    chat_history_messages = retrieve_data(current_session_id + "_feedback")
    ...
    logger.info({"user_id": user_id, "user_language": language, "feedback messages": messages})
    ...
    logger.info({"user_id": user_id, "user_language": language, "message_to_the_user_ai_assistant": ai_assistant})
    # if " | " in ai_assistant:
    #     strip_index = ai_assistant.index(' | ')
    #     ai_assistant = ai_assistant[:strip_index]
    # if "[" in ai_assistant:
    ...
    chat_history_messages.append({"role": "user", "content": user_input})
    chat_history_messages.append({"role": "assistant", "content": ai_assistant})
    store_data(current_session_id + "_feedback", json.dumps(chat_history_messages))
    ...
    logger.info({"user_id": user_id, "user_language": language, "feedback content_response": content_response})
    return content_response
