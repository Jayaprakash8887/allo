...
def invoke_llm(user_id, language, current_session_id, user_input) -> GetContentResponse:
    ...
    if ai_assistant.lower().startswith("user_agreed") or ai_assistant.lower().startswith("'user_agreed'"):
...
def invoke_llm_feedback(user_id, language, current_session_id, user_input) -> GetContentResponse:
    ...
    if "<emotion_category>" in ai_assistant:
        ...
        ai_assistant = ai_assistant[:strip_index].replace("message_to_user:", "").strip()
